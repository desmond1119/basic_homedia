...
const NotFoundView = memo(({ message }: { message: string }) => (
  <div className="min-h-screen bg-black flex items-center justify-center">
    <motion.div
...
export const ProviderProfilePage = memo(() => {
  ...
  const heroImage = useMemo(() => 
    currentProfile?.portfolios[0]?.imageUrl, 
    [currentProfile]
  );
  ...
  const providerStats = useMemo(() => ({
    ...
    completedProjects: currentProfile?.completedProjects ?? 0,
  }), [currentProfile]);
  ...
  useEffect(() => {
    if (providerId) {
      ...
      void dispatch(fetchPortfolios({ filters: { userId: providerId, status: 'approved' }, limit: 6 }));
    }
  }, [dispatch, providerId]);
  ...
  if (!currentProfile) {
    return <NotFoundView message={t('provider.notFound')} />;
  }
    ...
    <div className="min-h-screen bg-black font-['Inter']">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div
          ...
          className={`${heroImage ? '-mt-40' : 'pt-20'} relative`}
        >
        ...
        {portfolios.length > 0 && (
          ...
          >
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                onClick={() => navigate(`/portfolios?provider=${providerId}`)}
...
ProviderProfilePage.displayName = 'ProviderProfilePage';
